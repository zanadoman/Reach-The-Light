...
map::map()
{
    ...
    if (memory::LoadTo("saves/map.save", this->Raw, sizeof(uint8) * MAP_X * MAP_Y) + sizeof(uint8))
    {
        this->Reset();
    }
...
map::~map()
{
    memory::Save(this->Raw, sizeof(uint8) * MAP_X * MAP_Y + sizeof(uint8), "saves/map.save");
}
